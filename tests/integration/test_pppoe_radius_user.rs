use punglios::pppoe::auth::{
    self, MockRadiusBackend, RadiusAttribute, RadiusClient, RadiusPacket, UserRecord,
    ATTR_FRAMED_IP_ADDRESS, ATTR_SERVICE_TYPE, ATTR_WISPR_BANDWIDTH_MAX_DOWN,
    ATTR_WISPR_BANDWIDTH_MAX_UP, ACCESS_ACCEPT,
};
use punglios::pppoe::discovery::{
    DiscoveryState, MockPppoeBackend, PppoeClient, PppoeServer, PppoeServerConfig, PppoeBackend,
};
use punglios::pppoe::types::{
    AuthProtocol, PppoeClientConfig, PppoePacket, Tag, TAG_AC_COOKIE, TAG_AC_NAME, TAG_HOST_UNIQ,
    TAG_SERVICE_NAME,
};
use punglios::pppoe::session::{LCP_CONFIGURE_ACK, PppNegotiation, PppState, PPP_LCP};
use punglios::user::{
    BandwidthProfile, MockUserBackend, User, UserBackend, UserManager, UserPackage,
};
...
fn setup_pppoe_server(backend: MockPppoeBackend) -> PppoeServer<MockPppoeBackend> {
    ...
    let server = PppoeServer::new(backend, cfg, SERVER_MAC);
    server
}
...
async fn test_pppoe_discovery_with_radius_auth() {
    ...
    assert_eq!(pado.packet.code, auth::PADO);
    assert_eq!(
    ...
    assert_eq!(pads.packet.code, auth::PADS);
...
async fn test_multiple_clients_connect_sequentially() {
    ...
    for client_num in 0..3 {
        let cfg = PppoeClientConfig {
            ...
            auth_protocol: AuthProtocol::Pap,
            host_uniq: Some(vec![client_num as u8; 3]),
        };
...
async fn test_lcp_followed_by_pap_after_pppoe() {
    ...
    let mut neg = PppNegotiation::new_client(
        backend,
        "user1",
        "pass1",
        AuthProtocol::Pap,
    ...
    let mut server_neg = PppNegotiation::new_server(
        MockPppoeBackend::new(),
        Ipv4Addr::new(10, 0, 0, 1),
        vec![],
    );
